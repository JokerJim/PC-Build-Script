...
function Invoke-ApplyLayoutModification {
    ...
    if ([string]::IsNullOrWhiteSpace($XmlPath) -or -not (Test-Path $XmlPath)) {
        Write-Log "    LayoutModification.xml not found at: $XmlPath" ([System.Drawing.Color]::Yellow)
        return
    }
    try {
        $boot   = Get-Partition | Where-Object { $_.IsBoot -eq $true }
        $OSDISK = $boot.DriveLetter + ":"
        Import-StartLayout -LayoutPath $XmlPath -MountPath "$OSDISK\" -ErrorAction Stop
        Write-Log "    Layout modification applied from: $XmlPath"
        Write-Log "    Note: layout changes take effect for new user profiles only." ([System.Drawing.Color]::Cyan)
    } catch {
...
function Show-MainForm {
    ...
    $lblLayoutSrc.Text      = if (Test-Path "C:\Pirum\xml\LayoutModification.xml") { "    Found: C:\Pirum\xml\LayoutModification.xml" } else { "    Not found: C:\Pirum\xml\LayoutModification.xml  (will be skipped at run time)" }
    $lblLayoutSrc.ForeColor = if (Test-Path "C:\Pirum\xml\LayoutModification.xml") { $clrGreen } else { $clrGold }
    ...
    $btnRun.Add_Click({
        ...
        try {
            ...
            if ($cbLayoutMod.Checked) {
                Invoke-ApplyLayoutModification -XmlPath "C:\Pirum\xml\LayoutModification.xml"
            }
