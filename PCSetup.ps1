...
function Write-Log {
    ...
    $lb = if ($script:LogBox) { $script:LogBox } else { $null }
    if (-not $lb) { return }
...
function Show-MainForm {
    ...
    # Create LogBox first so it exists before being added to pnlLog
    $script:LogBox         = New-Object System.Windows.Forms.RichTextBox
    $logBox                = $script:LogBox   # local ref for closure capture
    ...
    # Build the header TableLayoutPanel
    ...
    # Add to pnlLog: LogBox first (Dock=Fill), then tblLogHdr (Dock=Top).
    # WinForms docks in reverse-add order so Top is processed before Fill.
    ...
    $btnRun.Add_Click({
        ...
        # Normal start
        $script:CancelRequested = $false
        $script:LogBox = $logBox   # ensure script: scope has the reference
        $btnRun.Text      = "CANCEL"
        ...
        $rs.SessionStateProxy.SetVariable("script_LogBox",      $logBox)
...
#          Add order: LogBox first (Dock=Fill), tblLogHdr second (Dock=Top).
#          WinForms processes dock in reverse-add order so Top reserves its
#          22px first and Fill takes the remaining space correctly.
#
# v1.58  - Root cause fix for log panel: $script:LogBox set inside Show-MainForm
#          is not visible from WinForms Add_Click scriptblock scope. Added
#          $logBox as a local closure variable (captured at creation time),
#          used for SetVariable into runspace and reassigned to $script:LogBox
#          at the start of Add_Click so all Write-Log calls resolve correctly.
#
...
#          filename on completion (if auto-save selected).#Requires -RunAsAdministrator
<#
.SYNOPSIS
    Pirum Consulting LLC - PC Setup & Configuration Tool v2.0
.DESCRIPTION
